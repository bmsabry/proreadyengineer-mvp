...
async def get_provider_quotes(
    ...
    quotes = result.scalars().all()
    # Bulk-fetch all RFQs for these quotes in ONE query (instead of N queries)
    rfq_ids = list({q.rfq_id for q in quotes})
    rfq_map = {}
    if rfq_ids:
        rfq_rows = await db.execute(select(RFQ).where(RFQ.id.in_(rfq_ids)))
        for rfq_row in rfq_rows.scalars().all():
            rfq_map[rfq_row.id] = rfq_row
    responses = []
    for q in quotes:
        ...
        if rfq:
            ...
            resp.rfq_status = str(rfq.rfq_status.value) if hasattr(rfq.rfq_status, "value") else str(rfq.rfq_status or "")
            resp.rfq_is_closed = rfq.is_closed or False
