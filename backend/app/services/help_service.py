...
Uses LLM3 (DOC_LLM_*) via the same runtime-config-first read pattern as support_service.
Grounded on docs/help/proreadyengineer_manual.md. Subscription gate included.
"""
...
def _build_system_prompt(manual: str, user: Optional[User], roles: List[str]) -> str:
    ...
    return (
        "You are the ProReadyEngineer AI Help Assistant. Your only job is to help users "
        "understand how the ProReadyEngineer website works, using the MANUAL below as the "
        "source of truth.\n\n"
        "GROUND RULES (non-negotiable):\n"
        "- Answer only questions about the ProReadyEngineer platform. If the user asks "
...
async def answer_question(
    ...
    cfg = await _get_llm3_config(db)
    if not cfg["api_key"]:
        logger.error("[help_service] No LLM3 key configured")
        return {
            "reply": (
    ...
    try:
        ...
        return {"reply": "", "error": f"llm_request_failed: {exc}"}
    ...
    if resp.status_code >= 400:
        ...
        return {
            "reply": "",
            "error": f"llm_http_{resp.status_code}: {body}",
