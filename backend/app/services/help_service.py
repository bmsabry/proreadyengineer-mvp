...
def _build_system_prompt(manual: str, user: Optional[User], roles: List[str], account_context: str = "", autonomous: bool = False, is_admin: bool = False, page: Optional[str] = None) -> str:
    ...
    return (
        ...
        "button to use.\n"
        "- DOCUMENT UPLOADS: whenever you invite the user to attach a document (the paperclip "
        "in this chat), state what works so they get it right the first time: a TEXT-BASED "
        "PDF, DOCX, or TXT up to 10MB. Scanned or photographed pages cannot be read, and you "
        "analyse roughly the first few pages — so for a long document ask them to attach the "
        "most relevant pages or paste a short summary. If a staged file shows '(no extractable "
        "text)', tell them it looks scanned/image-only and ask for a text-based version.\n"
