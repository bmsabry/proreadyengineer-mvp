...
const providerRFQ = {
  ...
  initiateProviderNda: (rfqId: string) =>
    apiClient.post<{ signing_url?: string; message?: string }>(`/provider/rfqs/${rfqId}/nda/signing-url`),
};
...
const admin = {
  ...
  getStatus: () =>
    apiClient.get<any>('/admin/stats'),
  listRFQs: (params?: { page?: number; page_size?: number; status?: string; since?: string; include_drafts?: boolean }) => 
...
export const helpApi = {
  ...
  action: async (type: string, opts?: { quote_id?: string; rfq_id?: string; attachments?: HelpUpload[]; project_description?: string; ticket_id?: string; page?: string; profile_updates?: Record<string, unknown> }): Promise<{ ok: boolean; message: string; link?: { href: string; label: string } | null }> => {
